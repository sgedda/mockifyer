...
- **Inline (test/debug only):** `X-Mockifyer-Include-Trace: 1` wraps JSON as `{ data, mockifyerTrace }` with in-process hops (no Redis). Optional `X-Mockifyer-Include-Trace-Bodies: 1`. Include-trace headers are forwarded downstream; nested `{ data, mockifyerTrace }` responses are unwrapped and child hops merged into the parent trace (parent hop first). Internal dashboard `/api/proxy` POSTs are always bypassed and omitted from hops. Both services need a recent core that supports inline trace.
