#!/usr/bin/env ts-node
/**
 * React Native Build Script Example
 * 
 * This script generates a TypeScript file from recorded mock data
 * for bundling with your React Native app.
 * 
 * Usage:
 *   ts-node examples/react-native-build-script.ts
 *   or add to package.json: "generate:build-data": "ts-node scripts/generate-build-data.ts"
 * 
 * Workflow:
...
// Configuration
const MOCK_DATA_PATH = path.join(__dirname, '../../mock-data'); // Adjust to your mock-data directory
const OUTPUT_PATH = path.join(__dirname, '../../assets/mock-data.ts'); // Output TypeScript file
