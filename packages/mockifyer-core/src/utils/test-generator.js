...
class TestGenerator {
     ...
     */
    analyzeMock(mockData, httpClientType = 'fetch') {
     ...
     */
    generateJestTest(testInfo, options, testFilePath) {
        var _a;
 ...
 *     const posts = await fetchUserPosts(${((_a = testInfo.queryParams) === null || _a === void 0 ? void 0 : _a.userId) || 'userId'});
...
`.trim();
     ...
     */
    generateVitestTest(testInfo, options, testFilePath) {
        var _a;
        const testName = this.generateTestName(testInfo);
        const setupCode = options.includeSetup !== false ? this.generateSetup(testInfo, options, testFilePath) : '';
        const requestCode = this.generateRequestCode(testInfo, options);
...
`.trim();
     ...
     */
    generateMochaTest(testInfo, options, testFilePath) {
        var _a;
        const testName = this.generateTestName(testInfo);
        const setupCode = options.includeSetup !== false ? this.generateSetup(testInfo, options, testFilePath) : '';
        const requestCode = this.generateRequestCode(testInfo, options);
...
`.trim();
     ...
     */
    generateSetup(testInfo, options, testFilePath) {
        ...
        if (!relativePath || relativePath === '.' || relativePath === '..') {
            ...
            const testDepth = normalizedTestDir.split(path.sep).filter((p) => p && p !== '.').length;
     ...
     */
    generateRequestCode(testInfo, options) {
     ...
     */
    generateGETRequest(testInfo, options) {
     ...
     */
    generatePOSTRequest(testInfo, options) {
     ...
     */
    generatePUTRequest(testInfo, options) {
        const url = testInfo.url;
        const body = this.formatRequestBody(testInfo.requestBody);
        if (testInfo.usesAxios) {
     ...
     */
    generatePATCHRequest(testInfo, options) {
        const url = testInfo.url;
        const body = this.formatRequestBody(testInfo.requestBody);
        if (testInfo.usesAxios) {
     ...
     */
    generateDELETERequest(testInfo, options) {
        const url = this.formatUrl(testInfo.url, testInfo.queryParams);
        if (testInfo.usesAxios) {
     ...
     */
    generateGraphQLRequest(testInfo, options) {
        const url = testInfo.url;
     ...
     */
    generateAssertions(testInfo, options) {
        const assertions = [];
     ...
     */
    determineTestFilePath(mockData, options) {
        const testInfo = this.analyzeMock(mockData, options.httpClientType);
