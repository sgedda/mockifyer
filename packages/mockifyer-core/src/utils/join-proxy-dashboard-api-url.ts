...
const DASHBOARD_PLUMBING_PATHS: ReadonlyArray<{ suffix: string; includeSubpaths: boolean }> = [
  ...
  { suffix: '/api/atlas', includeSubpaths: true },
];
...
/**
 * True when `url` targets dashboard plumbing (`/api/proxy`, `/api/network-events`, `/api/atlas`).
...
export function isMockifyerDashboardPlumbingUrl(url: string | null | undefined): boolean {
  ...
  try {
    ...
    return /(?:^|\/)api\/(?:proxy|network-events|atlas)(?:\/[^?#]*)?(?:\?|#|$)/i.test(url);
  }
}
