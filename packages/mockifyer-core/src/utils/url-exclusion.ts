 ...
 *
 * Dashboard plumbing (`/api/proxy`, `/api/network-events`) is always bypassed (even when
 * `excludedUrls` replaces defaults) so internal POSTs are never re-intercepted, recorded,
 * or shown as user-visible hops.
 */
