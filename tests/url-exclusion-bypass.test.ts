...
describe('shouldBypassMockifyerForUrl', () => {
  ...
  it('always bypasses dashboard /api/atlas even when custom excludedUrls replace defaults', () => {
    expect(
      shouldBypassMockifyerForUrl('http://localhost:3002/api/atlas/events', ['only.example.com'])
    ).toBe(true);
    expect(
      shouldBypassMockifyerForUrl('https://host/mockifyer/api/atlas/usage', ['only.example.com'])
    ).toBe(true);
  });
});
