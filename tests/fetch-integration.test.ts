...
describe("Mockifyer Fetch Integration", () => {
  ...
  it("should record mock, read it, and delete it", async () => {
    ...
    try {
      ...
      const mockFile = files.find(
        (file) =>
          file.includes("GET") &&
          (file.includes("api_example_com_record-test") ||
            file.includes("api_example_com_record_test")),
      );
      ...
      // The error message might be "fetch failed" or "Network error" depending on Node.js version
      await expect(newClient.get(testUrl)).rejects.toThrow();
