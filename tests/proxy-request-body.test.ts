...
describe('proxy request body serialization', () => {
  ...
  it('round-trips Node Buffer bodies as raw upstream bytes', async () => {
    const requestBody = Buffer.from([0xde, 0xad, 0xbe, 0xef]);
    const serialized = (await serializeProxyRequestBody(requestBody, {
    ...
    expect((upstream.body as Buffer).equals(requestBody)).toBe(true);
  ...
  it('serializes Blob bodies as raw bytes', async () => {
    const bytes = Uint8Array.from([0x00, 0xff, 0x89, 0x50]);
  ...
  it('serializes native FormData with file parts as multipart raw bytes', async () => {
    ...
    expect(multipartBody).toContain('\xde\xad\xbe\xef');
  });
