...
describe('proxy request body serialization', () => {
  ...
  it('serializes Node Buffer bodies as raw base64 proxy payloads', async () => {
    const source = Buffer.from([0, 1, 2, 255]);
    const serialized = (await serializeProxyRequestBody(source, {
    ...
    expect(serialized.data).toBe(source.toString('base64'));

    const upstream = buildProxyUpstreamBodyInit(serialized, {}, 'POST');
    expect(Buffer.isBuffer(upstream.body)).toBe(true);
    expect((upstream.body as Buffer).equals(source)).toBe(true);
  });
